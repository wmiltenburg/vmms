...
<?php
    ...
    $user = $_GET['user'];
        ...
        if($database->affectedRows() == 1){
            ...
            $database->getQuery($query);
            if($database->affectedRows() == 1){
                echo "<br/>User exists and trying to delete the order from the database <br />";

                $query = sprintf("DELETE FROM orders WHERE username = '$user'");
                $database->doQuery($query);


                echo "<br/>Order from user $user is removed<br />";
